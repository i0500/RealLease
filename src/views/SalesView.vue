...
const desktopColumns = [
  ...
  },
  {
    ...
    align: 'center' as const,
    render: (row: SaleContract) => {
      return row.downPayment > 0 ? formatCurrency(row.downPayment) : '-'
    }
  },
  {
    title: '계약금2',
    ...
    align: 'center' as const,
    render: (row: SaleContract) => {
      return row.downPayment2 > 0 ? formatCurrency(row.downPayment2) : '-'
    }
  },
  {
    title: '중도금',
  ...
  },
  {
    ...
    align: 'center' as const,
    render: (row: SaleContract) => {
      return row.finalPayment > 0 ? formatCurrency(row.finalPayment) : '-'
    }
  },
  {
    title: '합계',
    ...
    width: 120,
    align: 'center' as const,
    render: (row: SaleContract) => formatCurrency(row.totalAmount)
  },
  {
    title: '계약형식',
    key: 'contractFormat',
      ...
      >
        ...
        <div class="sale-info space-y-3">
          ...
          <div v-if="sale.downPayment > 0" class="info-row">
            ...
            <span class="value font-bold text-blue-600">{{ formatCurrency(sale.downPayment) }}</span>
          </div>

          <div v-if="sale.downPayment2 > 0" class="info-row">
            <span class="label">💰 계약금2</span>
            <span class="value font-bold text-blue-600">{{ formatCurrency(sale.downPayment2) }}</span>
          </div>

          <div v-if="sale.interimPayment1 > 0" class="info-row">
            <span class="label">💳 중도금1</span>
            <span class="value font-semibold text-purple-600">{{ formatCurrency(sale.interimPayment1) }}</span>
          </div>

          <div v-if="sale.interimPayment2 > 0" class="info-row">
            <span class="label">💳 중도금2</span>
            <span class="value font-semibold text-purple-600">{{ formatCurrency(sale.interimPayment2) }}</span>
          </div>

          <div v-if="sale.interimPayment3 > 0" class="info-row">
            <span class="label">💳 중도금3</span>
            <span class="value font-semibold text-purple-600">{{ formatCurrency(sale.interimPayment3) }}</span>
          </div>

          <div v-if="sale.finalPayment > 0" class="info-row">
            <span class="label">💵 잔금</span>
            <span class="value font-bold text-orange-600">{{ formatCurrency(sale.finalPayment) }}</span>
          </div>

          <div class="info-row border-t-2 border-gray-300 pt-2 mt-2">
            <span class="label font-bold">📊 합계</span>
            <span class="value font-bold text-green-600 text-lg">{{ formatCurrency(sale.totalAmount) }}</span>
          </div>
